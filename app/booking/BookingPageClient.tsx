...
function FavoriteButton({
    ...
    return (
        <button
            onClick={handleClick}
            disabled={pending}
            className={className || "absolute top-2 right-2 w-7 h-7 bg-white rounded-full flex items-center justify-center shadow hover:scale-110 transition disabled:opacity-60"}
        ...
        return (
            <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 backdrop-blur-sm">
                <div className="bg-white rounded-2xl shadow-2xl p-8 max-w-sm w-full mx-4 text-center">
                    ...
                    <h3 className="text-xl font-bold text-gray-900 mb-2">Đặt phòng thành công!</h3>
                    <p className="text-gray-500 text-sm mb-6">Thông tin đặt phòng đã được lưu. Bạn có thể xem trong mục Quản lý đặt phòng.</p>
                    <div className="flex gap-3">
                        <button onClick={onClose} className="flex-1 border border-gray-300 text-gray-700 text-sm font-semibold py-2.5 rounded-xl hover:bg-gray-50 transition">Đóng</button>
                        <button onClick={() => router.push("/account/bookings")} className="flex-1 bg-green-600 text-white text-sm font-semibold py-2.5 rounded-xl hover:bg-green-700 transition">Xem đặt phòng</button>
                    </div>
                </div>
            </div>
        );
                ...
                <div className="p-5 space-y-4">
                    ...
                    <div className="grid grid-cols-2 gap-3">
                        <div className="border border-gray-200 rounded-xl p-3 relative">
                            <span className="text-[10px] text-gray-400 block mb-1">Thời gian nhận</span>
                            <DateField label="" value={checkIn} onChange={setCheckIn} />
                        </div>
                        <div className="border border-gray-200 rounded-xl p-3 relative">
                            <span className="text-[10px] text-gray-400 block mb-1">Thời gian trả</span>
                            <DateField label="" value={checkOut} onChange={setCheckOut} />
                        </div>
                    </div>

                    {/* Guests */}
                    <div className="flex items-center justify-between border border-gray-200 rounded-xl px-4 py-3">
                        <span className="text-sm text-gray-700">Số khách <span className="text-gray-400 text-xs">(tối đa {room.bedroomCount ? room.bedroomCount + 2 : 3} khách)</span></span>
                        <div className="flex items-center gap-3">
                            <button
                                onClick={() => setGuests(g => Math.max(1, g - 1))}
                                className="w-7 h-7 rounded-full border border-gray-300 flex items-center justify-center text-gray-500 hover:border-green-500 hover:text-green-600 transition font-bold text-sm"
                            >−</button>
                            <span className="text-sm font-semibold text-gray-900 w-4 text-center">{guests}</span>
                            <button
                                onClick={() => setGuests(g => Math.min(room.bedroomCount ? room.bedroomCount + 2 : 3, g + 1))}
                                className="w-7 h-7 rounded-full border border-gray-300 flex items-center justify-center text-gray-500 hover:border-green-500 hover:text-green-600 transition font-bold text-sm"
                            >+</button>
                        </div>
                    </div>
                    ...
                    <div>
                        <input
                            ...
                            className="w-full border border-gray-200 rounded-xl px-4 py-3 text-sm placeholder-gray-400 focus:outline-none focus:border-green-400 transition"
                        />
                        <p className="text-[11px] text-gray-400 mt-1 ml-1">Chức năng mã giảm giá sẽ sớm được hỗ trợ.</p>
                    </div>

                    {/* Price breakdown */}
                    <div>
                        <p className="text-sm font-semibold text-gray-700 mb-2">Đơn giá chi tiết</p>
                        {nights > 0 ? (
                            <div className="space-y-1 text-sm text-gray-600">
                                <div className="flex justify-between">
                                    <span>{room.price.toLocaleString("vi-VN")}đ × {nights} đêm</span>
                                    <span>{total.toLocaleString("vi-VN")}đ</span>
                                </div>
                            </div>
                        ) : (
                            <p className="text-xs text-gray-400">Chọn ngày nhận và trả phòng để xem giá.</p>
                        )}
                    </div>
                    ...
                    <div className="flex justify-between items-center pt-1">
                        ...
                        <span className="font-bold text-green-600 text-base">{total > 0 ? `${total.toLocaleString("vi-VN")}đ` : "0đ"}</span>
                    </div>
                    ...
                    <button
                        onClick={handleBook}
                        disabled={loading}
                        className="w-full bg-green-600 hover:bg-green-700 disabled:bg-green-400 text-white font-bold py-3 rounded-xl transition text-sm"
                    >
                        {loading ? "Đang xử lý..." : isLoggedIn ? "Đặt phòng" : "Đăng nhập để đặt phòng"}
                    </button>
                </div>
            </div>
        </div>
...
export default function HomePageClient({
    ...
    }, [homestays, selectedBranch, searchTerm]);
